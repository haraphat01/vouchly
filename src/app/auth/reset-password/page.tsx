    ...
    <div style={{ minHeight: '100vh', background: 'var(--paper)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <h1 style={{ fontSize: '1.8rem', marginBottom: '0.5rem' }}>Set a new password</h1>
          <p style={{ color: 'var(--ink-muted)', fontSize: '0.95rem' }}>Choose a strong password for your account.</p>
        </div>
        ...
        {done ? (
          <div className="card" style={{ textAlign: 'center', padding: '2.5rem' }}>
            ...
            <p style={{ color: 'var(--ink-muted)', fontSize: '0.9rem' }}>Redirecting you to the dashboard…</p>
          </div>
        ) : (
          <div className="card">
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
              <div>
                ...
                <div style={{ position: 'relative' }}>
                  <input className="input" type={show ? 'text' : 'password'} value={password} onChange={e => setPassword(e.target.value)} placeholder="At least 8 characters" minLength={8} required style={{ paddingRight: '2.75rem' }} />
              ...
              <div>
                ...
                <input className="input" type={show ? 'text' : 'password'} value={confirm} onChange={e => setConfirm(e.target.value)} placeholder="Repeat your password" required />
              </div>
              ...
              <button type="submit" className="btn btn-primary" disabled={loading} style={{ justifyContent: 'center' }}>
                {loading ? <><Loader2 size={15} style={{ animation: 'spin 1s linear infinite' }} /> Updating…</> : 'Set new password'}
              </button>
            </form>
          </div>
        )}

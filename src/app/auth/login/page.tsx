    ...
    <div style={{ minHeight: '100vh', background: 'var(--paper)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem' }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: '2rem' }}>
          ...
          <h1 style={{ fontSize: '1.8rem', marginBottom: '0.5rem' }}>{t('login_title')}</h1>
          <p style={{ color: 'var(--ink-muted)', fontSize: '0.95rem' }}>{t('login_sub')}</p>
        </div>
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '1.1rem' }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: '0.35rem' }}>
                <label className="label" style={{ marginBottom: 0 }}>{t('password')}</label>
                <Link href="/auth/forgot-password" style={{ fontSize: '0.8rem', color: 'var(--brand)', textDecoration: 'none' }}>{t('forgot_password')}</Link>
              </div>
            ...
            <button type="submit" className="btn btn-primary" disabled={loading} style={{ marginTop: '0.25rem', justifyContent: 'center' }}>
              {loading ? <><Loader2 size={16} style={{ animation: 'spin 1s linear infinite' }} /> {t('signing_in')}</> : t('sign_in')}
            </button>
          </form>
        </div>
        ...
        <p style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '0.9rem', color: 'var(--ink-muted)' }}>
          {t('no_account')}{' '}
          <Link href="/auth/signup" style={{ color: 'var(--brand)', fontWeight: 600, textDecoration: 'none' }}>{t('sign_up_free')}</Link>
        </p>
      </div>
    </div>
  )
